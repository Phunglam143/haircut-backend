...
@Repository
public interface HaircutServiceRepository extends JpaRepository<HaircutService, Long> {
    List<HaircutService> findByActive(boolean active);
    List<HaircutService> findByNameContainingIgnoreCase(String name);
    List<HaircutService> findByPriceBetween(double minPrice, double maxPrice);
} 